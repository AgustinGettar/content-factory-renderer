...
async function generatePlan(request, { apiKey, fetchImpl, model }) {
  const response = await fetchImpl("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      text: {
        // The canonical Episode Plan intentionally contains open parameter/state maps,
        // which strict Structured Outputs rejects. JSON mode guarantees parseable JSON;
        // the complete local AV2 schema and semantic validators remain authoritative.
  ...
  let payload;
  try {
    payload = JSON.parse(outputText(raw));
  } catch (error) {
...
export async function runCanonicalBenchmark({
  ...
  try {
    ...
    await integration.recordFailure({
      ...
      failureId: `benchmark:${resolved.request_hash}:1`,
      context,
      idempotencyKey: AV2_BENCHMARK_IDEMPOTENCY_KEY,
    });
    throw error;
  ...
  await integration.store.update(accepted.artifact_id, {
    generation_metadata: { ...row.generation_metadata, ...generated.metadata },
  });
  return {
    ...accepted,
    provider_calls: 1,
