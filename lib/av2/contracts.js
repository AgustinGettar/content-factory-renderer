...
function collectTimeRefs(value, target = []) {
  if (Array.isArray(value)) {
    value.forEach((entry) => collectTimeRefs(entry, target));
  } else if (value && typeof value === "object") {
    if (typeof value.anchor === "string" && Object.keys(value).every((key) => ["anchor", "offset_seconds"].includes(key))) {
      target.push(value.anchor);
    } else {
      Object.values(value).forEach((entry) => collectTimeRefs(entry, target));
    }
  }
  return target;
}
...
export function validateEpisodePlan(plan, { throwOnError = true } = {}) {
  const errors = [];
  if (!validateEpisodeShape(plan)) errors.push(...schemaErrors(validateEpisodeShape.errors));
  ...
  const plannedDuration = scenes.reduce((sum, scene) => sum + scene.duration_target_seconds, 0)
    - episode.transitions.reduce((sum, edge) => sum + edge.overlap_frames / 30, 0);
  ...
  scenes.forEach((scene, sceneIndex) => {
    ...
    collectTimeRefs(scene).forEach((anchor) => {
      if (!localAnchors.has(anchor)) addSemanticError(errors, `/scenes/${sceneIndex}`, `unknown local time anchor ${anchor}`, "timing_reference");
    });
