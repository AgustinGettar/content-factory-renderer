...
import { CreativeValidationError, EPISODE_PLAN_SCHEMA, IDEA_SET_SCHEMA } from "./contracts.js";
...
export const AV2_LLM_IDEA_TRANSPORT_VERSION = "av2-llm-idea-transport/1";
export const AV2_LLM_IDEA_TRANSPORT_SCHEMA_NAME = "av2_llm_idea_transport_v1";
...
export const AV2_LLM_IDEA_TRANSPORT_SCHEMA = Object.freeze({
  type: "object",
  additionalProperties: false,
  properties: {
    transport_version: { type: "string", const: AV2_LLM_IDEA_TRANSPORT_VERSION },
    idea_set: transportize(IDEA_SET_SCHEMA),
  },
  required: ["transport_version", "idea_set"],
});
...
const validateIdeaTransportShape = ajv.compile(AV2_LLM_IDEA_TRANSPORT_SCHEMA);
...
export function validateIdeaTransport(value, { throwOnError = true } = {}) {
  const errors = validateIdeaTransportShape(value) ? [] : schemaErrors(validateIdeaTransportShape.errors);
  if (errors.length && throwOnError) throw new CreativeValidationError("idea_transport_invalid", errors);
  return { ok: errors.length === 0, errors };
}
...
function normalizeIdeaNode(value, schema, path) {
  if (schema?.type === "array") return value.map((entry, index) => normalizeIdeaNode(entry, schema.items, `${path}/${index}`));
  if (schema?.type === "object" && schema.properties) {
    const domainRequired = new Set(schema.required || []);
    const result = {};
    for (const [key, propertySchema] of Object.entries(schema.properties)) {
      if (value[key] === null && !domainRequired.has(key)) continue;
      result[key] = normalizeIdeaNode(value[key], propertySchema, `${path}/${key}`);
    }
    return result;
  }
  return structuredClone(value);
...
export function transportToIdeaDomain(transport) {
  validateIdeaTransport(transport);
  return normalizeIdeaNode(transport.idea_set, IDEA_SET_SCHEMA, "/idea_set");
}
...
function denormalizeIdeaNode(value, schema, path) {
  if (schema?.type === "array") return value.map((entry, index) => denormalizeIdeaNode(entry, schema.items, `${path}/${index}`));
  if (schema?.type === "object" && schema.properties) {
    const domainRequired = new Set(schema.required || []);
    const result = {};
    for (const [key, propertySchema] of Object.entries(schema.properties)) {
      if (!Object.hasOwn(value, key)) {
        if (domainRequired.has(key)) throw new Error(`Missing required idea property ${path}/${key}`);
        result[key] = null;
      } else {
        result[key] = denormalizeIdeaNode(value[key], propertySchema, `${path}/${key}`);
      }
    }
    return result;
  }
...
export function ideaDomainToTransport(ideaSet) {
  return {
    transport_version: AV2_LLM_IDEA_TRANSPORT_VERSION,
    idea_set: denormalizeIdeaNode(ideaSet, IDEA_SET_SCHEMA, "/idea_set"),
  };
}
