...
export class Av2IntegrationError extends Error {
  constructor(code, message, {
    status = 400, recoverable = false, providerCalled = false, diagnostic = null,
  } = {}) {
    super(message);
...
function cleanProviderError(value) {
  ...
  return {
    ...
    code: clean(value.code, 100),
    param: clean(value.param, 160),
    message: clean(value.message, 500),
    request_id: clean(value.request_id, 160),
  };
}
...
export class Av2PipelineIntegration {
  ...
  async acceptPlan({
    requestHash, payload, context = {}, idempotencyKey, repairable = true, validationMetadata = {},
  }) {
    requireV2Gate(this.configuration, context);
    try {
      ...
      await this.#recordValidationFailure("episode_plan", requestHash, idempotencyKey, error, repairable);
      throw error;
    }
  }

  async persistProviderOutput({
    artifactType = "episode_plan", requestHash, generationAttempt, rawTransport,
    providerMetadata = {}, context = {}, idempotencyKey,
  }) {
    requireV2Gate(this.configuration, context);
    const row = await this.store.findByRequest(artifactType, requestHash);
    this.#assertOwner(row, idempotencyKey);
    if (Number(row.generation_attempt) !== Number(generationAttempt)) {
      throw new Av2IntegrationError("generation_attempt_conflict", "provider output attempt does not match the artifact", { status: 409 });
    }
    const rawTransportHash = contentHash(rawTransport);
    const saved = await this.store.saveProviderOutput({
      artifact_id: row.id,
      generation_attempt: Number(generationAttempt),
      request_hash: requestHash,
      request_payload_hash: providerMetadata.request_payload_hash || null,
      transport_version: String(rawTransport?.transport_version || providerMetadata.transport_version || "").slice(0, 100),
      provider: String(providerMetadata.provider || "openai").slice(0, 40),
      provider_response_id: providerMetadata.provider_response_id || null,
      provider_model: providerMetadata.model || null,
      provider_created_at: providerMetadata.provider_created_at || null,
      input_tokens: Number(providerMetadata.input_tokens || 0),
      output_tokens: Number(providerMetadata.output_tokens || 0),
      raw_transport: rawTransport,
      raw_transport_hash: rawTransportHash,
    });
    const providerCallCount = saved.created
      ? Math.max(Number(row.generation_metadata?.provider_call_count || 0) + 1, Number(generationAttempt))
      : Number(row.generation_metadata?.provider_call_count || 0);
    await this.store.update(row.id, {
      generation_metadata: {
        ...row.generation_metadata,
        provider_call_count: providerCallCount,
        provider_output_persisted: true,
        provider_transport_hash: rawTransportHash,
        provider_response_id: providerMetadata.provider_response_id || null,
        provider_model: providerMetadata.model || null,
        transport_version: rawTransport?.transport_version || providerMetadata.transport_version || null,
      },
    });
    log(this.logger, "provider_output_persisted", {
      artifact_id: row.id,
      request_hash: requestHash,
      generation_attempt: Number(generationAttempt),
      persistence_result: saved.created ? "stored" : "reused",
    });
    return {
      artifact_id: row.id,
      generation_attempt: Number(generationAttempt),
      raw_transport_hash: rawTransportHash,
      provider_call_count_total: providerCallCount,
      created: saved.created,
    };
  }

  async loadProviderOutput({
    artifactType = "episode_plan", requestHash, generationAttempt, context = {}, idempotencyKey,
  }) {
    requireV2Gate(this.configuration, context);
    const row = await this.store.findByRequest(artifactType, requestHash);
    this.#assertOwner(row, idempotencyKey);
    const output = await this.store.findProviderOutput(row.id, Number(generationAttempt));
    if (!output) throw new Av2IntegrationError("provider_output_not_found", "persisted provider output does not exist", { status: 404 });
    if (contentHash(output.raw_transport) !== output.raw_transport_hash) {
      throw new Av2IntegrationError("provider_output_hash_mismatch", "persisted provider output failed its hash", { status: 409 });
    }
    return output;
  }
  ...
  async recordFailure({
    artifactType, requestHash, code, recoverable = false, failureId, context = {}, idempotencyKey,
    providerCalled = false, providerError = null,
  }) {
    requireV2Gate(this.configuration, context);
    const row = await this.store.findByRequest(artifactType, requestHash);
    this.#assertOwner(row, idempotencyKey);
    ...
    const safeProviderError = cleanProviderError(providerError);
    ...
    const retry = Boolean(recoverable) && row.generation_attempt < MAX_AUTOMATIC_GENERATION_ATTEMPTS;
    const updated = await this.store.updateIfStatus(row.id, ["generating", "repairing"], {
      ...
      ...(providerCalled ? {
        generation_metadata: {
          ...
          provider_call_count: Math.max(
            ...
            Number(row.generation_attempt || 1),
          ),
        },
      } : {}),
    });
    ...
    log(this.logger, retry ? "generation_retry_allowed" : "generation_failed", {
      ...
      generation_attempt: retry ? row.generation_attempt + 1 : row.generation_attempt,
      ...(safeProviderError ? {
        provider_http_status: safeProviderError.http_status,
        provider_error_type: safeProviderError.type,
        provider_error_code: safeProviderError.code,
        provider_error_param: safeProviderError.param,
        provider_error_message: safeProviderError.message,
        provider_request_id: safeProviderError.request_id,
      } : {}),
    });
    return {
  ...
  async #resolve(artifactType, request, context, idempotencyKey, accept) {
    ...
    if (!row && context.benchmark_id === AV2_BENCHMARK_ID
        && this.store.findByIdempotency && this.store.rebindFailedRequest) {
      const prior = await this.store.findByIdempotency(artifactType, key);
      if (prior && !prior.content_hash && ["generating", "repairing", "failed"].includes(prior.status)) {
        const reopen = prior.status === "failed";
        if (reopen && Number(prior.generation_attempt || 1) >= MAX_GENERATION_ATTEMPTS) {
          throw new Av2IntegrationError("generation_limit_exceeded", `generation attempts are limited to ${MAX_GENERATION_ATTEMPTS}`, { status: 409 });
        }
        const previousFailures = Array.isArray(prior.generation_metadata?.previous_failures)
          ? prior.generation_metadata.previous_failures : [];
        const patch = reopen ? {
          status: "generating",
          validation_status: "pending",
          generation_attempt: Number(prior.generation_attempt || 1) + 1,
          generation_metadata: {
            ...prior.generation_metadata,
            previous_failures: prior.last_error
              ? [...previousFailures, { attempt: Number(prior.generation_attempt || 1), ...prior.last_error }]
              : previousFailures,
          },
        } : {};
        row = await this.store.rebindFailedRequest(prior.id, prior.request_hash, request.request_hash, patch);
        if (!row) throw new Av2IntegrationError("idempotency_conflict", "benchmark request changed during retry rebind", { status: 409 });
        log(this.logger, "failed_request_rebound", {
          artifact_id: row.id, request_hash: request.request_hash,
          generation_attempt: row.generation_attempt, persistence_result: "reused",
        });
      }
    }
    if (row?.status === "valid") {
    ...
    if (row) {
      ...
      return { ok: state !== "failed", operation: `resolve_${artifactType}`, state, cache_hit: false, replay: true, artifact_id: row.id, request_hash: request.request_hash, generation_attempt: row.generation_attempt, ...(state === "generate" ? { request } : {}), ...(state === "failed" ? { error: row.last_error_code } : {}) };
    }
    ...
    return { ok: true, operation: `resolve_${artifactType}`, state: "generate", cache_hit: false, replay: false, artifact_id: row.id, request_hash: request.request_hash, generation_attempt: row.generation_attempt, request };
  }

  async #accept(artifactType, requestHash, payload, context, idempotencyKey, accept, validationMetadata = {}) {
    const row = await this.store.findByRequest(artifactType, requestHash);
    this.#assertOwner(row, idempotencyKey);
    const accepted = accept(payload);
    const acceptedPayload = artifactType === "episode_plan"
      ? accepted.episode_plan
      : (accepted.idea_set || payload);
    const payloadHash = contentHash(acceptedPayload);
    if (row.status === "valid") {
    ...
    const updated = await this.store.updateIfStatus(row.id, ["generating", "repairing", "failed"], patch);
    if (!updated) {
      const concurrent = await this.store.findByRequest(artifactType, requestHash);
