...
import { MOUTH_FILE, validatePerformance, talkingExpression, headAlpha, bodyAlpha, headRotation } from "./lumi-performance.js";
...
export async function renderStoryScene({ imagePath, audioPath, textPath, outputPath, profile, production, timeline, performance = null, characterFiles = CHARACTER_FILES, mouthFile = MOUTH_FILE, fontPath = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", timeoutMs = 900000 }) {
  ...
  // Decode artwork once. Loop the prepared frame inside the filter graph instead
  // of decoding megabyte PNGs and rasterizing SVGs again on every frame.
  const still = file => args.push("-framerate",String(fps),"-i",file);
  const hold = `loop=loop=-1:size=1:start=0,setpts=N/${fps}/TB,trim=duration=${d}`;
  ...
  if (performance) {
    validatePerformance(performance,timeline);
    still(path.resolve(mouthFile));
  }
  const graph = [];
  graph.push(`[0:v]scale=${w*2}:${h*2}:force_original_aspect_ratio=increase,crop=${w*2}:${h*2},zoompan=z='1.025+0.012*(1-cos(PI*on/${Math.max(1,frames-1)}))':x='iw/2-iw/zoom/2':y='ih/2-ih/zoom/2':d=${frames}:s=${w}x${h}:fps=${fps},setsar=1[bg]`);
  const xpos = `(W-w)/2+${px(7)}*sin(2*PI*t/5.2)`, ypos = `${px(915)}+${px(6)}*sin(2*PI*t/3.8)`;
  if (performance) {
    const alpha = value => `geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='${value}'`;
    const maskedStill = value => `${alpha(value)},${hold}`;
    graph.push("[2:v]format=rgba,split=2[bodySrc][headSrc]");
    graph.push(`[bodySrc]${maskedStill(bodyAlpha)}[body]`);
    graph.push(`[headSrc]${maskedStill(headAlpha)}[headOpen]`);
    graph.push(`[3:v]format=rgba,${maskedStill(headAlpha)}[headBlink]`);
    graph.push("[headOpen][headBlink]overlay=0:0:enable='lt(mod(t+1.1,3.7),0.13)':format=auto[eyes]");
    graph.push(`[eyes][6:v]overlay=222:274:enable='not(gt(${talkingExpression(performance)},0))':format=auto:alpha=premultiplied[mouth]`);
    graph.push(`[mouth]rotate='${headRotation(p.role)}':c=none:ow=iw:oh=ih[head]`);
    graph.push(`[body][head]overlay=0:0:format=auto,scale=-1:${px(890)},rotate='0.008*sin(2*PI*t/4.8)':c=none:ow=iw:oh=ih[actor]`);
    graph.push(`[bg][actor]overlay=x='${xpos}':y='${ypos}':format=auto[blinking]`);
  } else {
    // Approved v2 previews keep their original whole-sprite transforms in HD.
    for (const [index,name] of [[2,"open"],[3,"blink"]]) {
      graph.push(`[${index}:v]format=rgba,scale=-1:${px(890)},${hold},rotate='0.008*sin(2*PI*t/4.8)':c=none:ow=iw:oh=ih[${name}]`);
    }
  ...
  const textFocus = ["label","letter"].includes(p.focus.kind) && !(p.role === "question" && p.focus.kind === "label");
  ...
  graph.push(`[4:v]format=rgba${focusText},scale=${px(880)}:-1,${hold},fade=t=in:st=0:d=0.2:alpha=1[focus]`);
  graph.push(`[blinking][focus]overlay=x=(W-w)/2:y='${px(350)}+${px(35)}*exp(-8*t)':format=auto[stage]`);
  let last = "stage";
  if (p.role === "answer" && p.focus.kind !== "count") {
    graph.push(`[5:v]format=rgba,scale=${px(74)}:-1,${hold},fade=t=in:st=0:d=0.1:alpha=1,fade=t=out:st=0.9:d=0.35:alpha=1[magic]`);
